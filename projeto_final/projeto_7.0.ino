...
void gira_motor(int pino_pulso, float medida, float posicao_atual){ // Função recebe o pino de pulso, medida bipada e a posição atual como parâmetros
  ...
  for (int i = 0; i < (qtd_passos * 4632.3); i++){ // O motor gira x vezes de acordo com a expressão anterior. Altere essa condição de acordo com seu referencial de medidas
    ...
    digitalWrite(pino_pulso, HIGH);
    delayMicroseconds(400);           // Velocidade de giro do motor durante as leituras de largura e altura (400 = Valor mais adequado para o motor girar razoavelmente rápido e sem ruídos)
    digitalWrite(pino_pulso, LOW);
  }
}
  ...
  for(contador = 0; contador < num_pedidos * 2; contador++){ // REPETE A LEITURA DE ETIQUETAS ATÉ A QUANTIDADE DE PEDIDOS (*2 PARA BATER O CÁLCULO)
    ...
    for (int i = 0; i < num_pedidos; i++) {
      ...
      if (codigos[i] == codigo) { // Verifica se o código de barras digitado ou bipado corresponde à alguns dos códigos no vetor códigos[]
        ...
        medida_largura = larguras[i]; // Aqui vamos armazenar o valor correspondente a largura do respectivo índice
        ...
        sentido_rotacao(posicao_atual_largura, medida_largura); // Define o sentido de rotação do motor de acordo com os parâmetros da medida da largura
        
        segundo_ciclo = 0; // REINICIA O ESTADO DO BOTÃO PARA 0 - IMPORTANTE PARA COMEÇAR O SEGUNDO LOOP (ALTURA)

        gira_motor(pino_pulso, posicao_atual_largura, medida_largura); // Define a velocidade e quantidade de voltas do motor de acordo com os parâmetros atuais
      ...
      if (digitalRead(botao_altura) == LOW) { 
        ...
        for (int i = 0; i < num_pedidos; i++) {
          ...
          if (codigos[i] == codigo) {  // Verifica se o código de barras digitado ou bipado corresponde à alguns dos códigos no vetor códigos[]
            ...
            medida_altura = alturas[i]; // Aqui vamos armazenar o valor correspondente a altura do respectivo índice
            ...
            sentido_rotacao(posicao_atual_altura, medida_altura);  // Define o sentido de rotação do motor de acordo com os parâmetros da medida da altura

            gira_motor(pino_pulso, posicao_atual_altura, medida_altura);  // Define a velocidade e quantidade de voltas do motor de acordo com os parâmetros atuais
