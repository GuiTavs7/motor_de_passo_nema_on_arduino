...
int num_pedidos = 100;                                    // Número de pedidos (etiquetas) - altere conforme necessário                                                        
...
void inicio(float &posicao_atual){   //"&" torna a variável posição_atual intercambiável entre a função "afasta_motor()" e void loop() 
  ...
  if(estado_botao_inicio == LOW){ // Se o botão de início for pressionado:
    ...
    for (int x = 0; x < (0.331 * 4545.971133083305 ); x++) {   // POSICAO DE CALIBRAÇÃO - POSICAO DESEJADA = 4.731 - 4.40 = 0.331 * constante (4545.971133083305)
...
void loop() { 
  ...
  for(contador = 0; contador < num_pedidos * 2; contador++){ // REPETE A LEITURA DE ETIQUETAS ATÉ A QUANTIDADE DE PEDIDOS (*2 PARA BATER O CÁLCULO)
    ...
    // 10.1) 1º CASO - BOTÃO DE AFASTAR O MOTOR APERTADO
    ...
    if (Serial.available() > 0) { // Caso dados sejam recebidos na porta serial:
      ...
      // 10.3) SEGUNDO CICLO - ALTURAS - SÓ PROSSEGUE SE O BOTÃO PARA LER A MEDIDA DA ALTURA FOR PRESSIONADO
