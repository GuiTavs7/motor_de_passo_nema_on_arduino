...
float alturas[] = {1.0, 3.0, 7.0}; //Vetor de alturas correspondentes às peças conforme os códigos de barras
...
void loop() {
  ...
  for(contador = 0; contador < num_pedidos * 2; contador++){ // REPETE A LEITURA DE ETIQUETAS ATÉ A QUANTIDADE DE PEDIDOS (*2 PARA BATER O CÁLCULO)
   ...
   // -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    // VERIFICAÇÃO DE PARADA TOTAL ANTES DO SEGUNDO CICLO - ALTURA
