...
const int botao_afasta_motor = 1;   // CONFIGURA O PINO DO BOTÃO QUE AFASTA MOTOR
...
void setup() {
  ...
  pinMode(botao_afasta_motor, INPUT_PULLUP); // DEFINE O BOTÃO PARADA TOTAL COMO ENTRADA E COM RESISTOR INTERNO
...
void loop() { 
  ...
  for(contador = 0; contador < num_pedidos * 2; contador++){ // REPETE A LEITURA DE ETIQUETAS ATÉ A QUANTIDADE DE PEDIDOS (*2 PARA BATER O CÁLCULO)
    ...
    Serial.println("\n Insira o número do código de barras ou aperte o botão de afastar o motor! ");
     ...
     while (!Serial.available() && digitalRead(botao_afasta_motor) == HIGH) {    // Verifica continuamente se dados foram recebidos na porta serial ou se o botão de afastar foi pressionado
