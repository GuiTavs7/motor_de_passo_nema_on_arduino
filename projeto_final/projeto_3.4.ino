...
float alturas[] = {4.6, 4.3, 4.3, 4.3, 4.4, 4.5, 4.65};                               // Vetor de alturas correspondentes às peças conforme os códigos de barras (em metros)
...
void inicio(float &posicao_atual){   //"&" torna a variável posição_atual intercambiável entre a função "afasta_motor()" e void loop() 
  ...
  float qtd_passos = abs((posicao_atual - 4.40)); // Substitua o 4.40 para sua posição inicial
    ...
    delayMicroseconds(1000);
    Serial.println("\n MOTOR INDO PARA O INÍCIO (SENTIDO HORÁRIO) \n");
...
void loop() {
  ...
  // 10.2) BOTÃO DE INÍCIO - LOOP SEMPRE COMEÇA AQUI!

  Serial.println("\n APERTE O BOTÃO DE INÍCIO OU O BOTÃO DE AFASTAR O MOTOR! \n");
  
  while ((digitalRead(botao_inicio) == HIGH) && digitalRead(botao_afasta_motor) == HIGH) {    // Espera até o botão de início ser pressionado
  ...
  // 10.3) ESPERA ATÉ APERTAR O BOTÃO DE AFASTAR MOTOR OU BIPAR PEÇA

  Serial.println("\n BIPE O CÓDIGO DE BARRAS OU APERTE O BOTÃO DE AFASTAR O MOTOR! ");

  while ((Serial.available() == 0) && digitalRead(botao_afasta_motor) == HIGH){

  }
  
  // LOOP INTERROMPIDO - BOTÃO AFASTAR MOTOR APERTADO OU ETIQUETA BIPADA

  // 10.4) 1º CASO - BOTÃO DE AFASTAR O MOTOR APERTADO
  ...
  if (Serial.available() > 0) { // Caso dados sejam recebidos na porta serial:
    ...
    // 10.6) SEGUNDO CICLO - ALTURAS - SÓ PROSSEGUE SE O BOTÃO PARA LER A MEDIDA DA ALTURA FOR PRESSIONADO
