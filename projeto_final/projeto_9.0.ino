...
int estado_botao_corte = HIGH;                            // Estado do botão de corte
...
void subir_prensa() {
  ...
  digitalWrite(pino_rele, HIGH); // PINO DO RELÉ HIGH = PRENSA SOBE!
...
void loop() { 
  ...
  for(contador = 0; contador < num_pedidos * 2; contador++){ // REPETE A LEITURA DE ETIQUETAS ATÉ A QUANTIDADE DE PEDIDOS (*2 PARA BATER O CÁLCULO)
    ...
    for (int i = 0; i < num_pedidos; i++) {
      ...
      if (codigos[i] == codigo) { // Verifica se o código de barras digitado ou bipado corresponde à alguns dos códigos no vetor códigos[]
        ...
        digitalWrite(pino_rele, LOW); // PINO DO RELÉ LOW = PRENSA ABAIXA!
  ...
  // 7) MOTOR VOLTA LENTAMENTE PARA A POSIÇÃO INICIAL APÓS "X" LEITURAS - RECALIBRAÇÃO!
  ...
  if (contador >= num_pedidos * 2){ 
    ...
    for(int x = 0; x < 80000; x++){ // Loop For para girar o motor no sentido anti-horário continuamente
      ...
      // 7.1) BOTÃO FIM DE CURSO - CASO SEJA NECESSÁRIO, REDUZ AINDA MAIS A VELOCIDADE DE RETORNO PARA O FIM DE CURSO
