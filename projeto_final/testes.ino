...
void gira_motor(int pino_pulso, float medida_lida, float posicao_atual){
  ...
  int passos_deceleracao = qtd_passos * 0.8; // 80% dos passos para começar a desacelerar
  int velocidade_atual = velocidade_inicial;

  for (int i = 0; i < (qtd_passos * 4553.215); i++){
    ...
    // Ajusta a velocidade com base na proximidade da posição final
    if (i >= passos_deceleracao) {

      float proporcao = (float)(i - passos_deceleracao) / (qtd_passos - passos_deceleracao);

      velocidade_atual = velocidade_inicial - proporcao * (velocidade_inicial - velocidade_final);

      if (velocidade_atual < velocidade_final) {
        
        velocidade_atual = velocidade_final;
      }
    }
