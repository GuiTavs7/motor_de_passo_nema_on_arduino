...
void calibra_motor(){
  ...
  if (estado_botao_parada_total == HIGH){ // Continua a calibração enquanto o botão de parada total não for pressionado
    ...
    delay(500); // PEQUENO ATRASO PARA NÃO MOVIMENTAR SIMULTANEAMENTE A PRENSA E O MOTOR
...
void gira_motor(int pino_pulso, float medida_lida, float posicao_atual){ 

  const int velocidade_inicial = 2400;   
  const int velocidade_final = 600;      
  const int passos_aceleracao = 600;     

  float qtd_passos = abs((posicao_atual - medida_lida)); 

  int velocidade_atual = velocidade_inicial; 
  int passos_restantes = qtd_passos * 4553.215;

  for (int i = 0; i < passos_restantes; i++){ 
    ...
    if (i < passos_aceleracao) {
      ...
      // Diminuir a velocidade gradualmente à medida que nos aproximamos do destino
      velocidade_atual += (velocidade_inicial - velocidade_final) / (passos_restantes - passos_aceleracao);
    }

    digitalWrite(pino_pulso, HIGH);
    delayMicroseconds(velocidade_atual);           
    digitalWrite(pino_pulso, LOW);
    
    passos_restantes--; // Decrementar o número de passos restantes
  }
}
