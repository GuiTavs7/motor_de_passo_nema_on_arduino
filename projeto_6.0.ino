...
// 0.2) FUNÇÃO ESCOLHE SENTIDO HORÁRIO OU ANTI-HORÁRIO DE ACORDO COM A COMPARAÇÃO ENTRE A POSIÇÃO ATUAL (MEDIDA ANTERIOR) E A NOVA ETIQUETA !!! 
...
// 0.3) FUNÇÃO DE GIRO DO MOTOR - MOVIMENTA O MOTOR 1 CICLO (1 CICLO = 800 PULSOS = 172,7mm = 17,27cm)
  ...
  for(contador = 0; contador < num_pedidos * 2; contador++){ // REPETE A LEITURA DE ETIQUETAS ATÉ A QUANTIDADE DE PEDIDOS (*2 PARA BATER O CÁLCULO)
    ...
    for (int i = 0; i < num_pedidos; i++) {
      ...
      if (codigos[i] == medida_largura) { 
        ...
        sentido_rotacao(posicao_atual, medida_largura);
        
        segundo_ciclo = 0; // REINICIA O ESTADO DO BOTÃO PARA 0 - IMPORTANTE PARA COMEÇAR O SEGUNDO LOOP (ALTURA)

        gira_motor(pino_pulso, posicao_atual, medida_largura);

        posicao_atual = medida_largura;  // POSICAO ATUAL RECEBE O VALOR DA MEDIDA LIDA PARA PRÓXIMAS COMPARAÇÕES
      ...
      if (digitalRead(botao_altura) == LOW) { 
        ...
        for (int i = 0; i < num_pedidos; i++) {
          ...
          if (alturas[i] == medida_altura) { 
            ...
            medida_altura = alturas[i]; // Na segunda volta armazenamos na quantidade de voltas o valor do vetor ALTURAS
            ...
            sentido_rotacao(posicao_atual, medida_altura);

            gira_motor(pino_pulso, posicao_atual, medida_altura);

            posicao_atual = medida_altura;  // POSICAO ATUAL RECEBE O VALOR DA MEDIDA LIDA PARA PRÓXIMAS COMPARAÇÕES
